...
Qt开发中，pro文件是对正工程所有源码，编译，资源，目录等的全方位配置的唯一方式，pro文件的编写非常重要
...
{配置
	...
	CONFIG
		...
		一下配置值控制编译标志：
		选项						说明
		release						项目以release模式构建。如果指定了debug，那么最后一个生效。
		debug						项目以debug模式构建。
		...
		bulid_all					如果指定了debug_and_release,默认情况下，该项目会构建为debug和release模式。
		autogen_procompile_source	自动生成一个.cpp文件，包含在.pro中指定的预编译头文件。
		ordered						使用subdirs模板时，此选项指定应该按照目录列表的顺序处理它们。
		procompile_header			可以在项目中使用预编译头文件的支持。
		warn_on						编译器应该输出尽可能多的警告。如果也指定了warn_off，最后一个生效。
		warn_off					编译器应该输出尽可能少的警告。
		execptions					启用异常支持。默认设置。
		execptions_off				禁用异常支持。
		rtti						启动RTTI支持。默认情况下，使用编译器默认。
		...
		c++14						启用c++14支持。如果编译器不支持c++14，，没有影响。默认情况下，支持是禁用的。
		dll							动态库(dll)
		staticlib					静态库
		plugin						插件
		
		当使用debug和release选项时(Windows下默认)，该项目将被处理三次；一次生成一个"meta"Makefile,另外两次生成Makefile.Debug和Makefile.Release.
		
	DEFINES
		qmake添加这个变量的值作为编译器C预处理宏(-D选项)。
	...
	FORMS
		指定UI文件(参考：Qt Designer Manual)在编译钱被uic处理。所有的构建这些UI文件所需的依赖、头文件和源文件都会自动被添加到项目中。
		例如：
		FORMS = mydialog.ui \
				mywidget.ui \
				myconfig.ui
	...
	LIBS
		指定链接到项目中的库列表。如果使用Unix -l(library)和 -L(library path)标志，在Windows上qmake正确处理库(也就是说，将库的完整路径传递给连接器)，
			库必须存在，qmake会寻找-l指定的库所在的目录。
		例如：
	...
	DISTFILES
		链接dll库的，libs不能链接.dll
			DISTFILES += libssl-1_1.dll //链接库
	...
	QT
		...
		如果你的项目时一个Qt Designer插件，使用uiplugin指定项目构建成库，但特定的Qt Designer插件支持，请参考：Building and Installing the Plugin。
	...
	RESOURCES
		指定资源文件(qrc)的名称，参考：Qt之资源系统 https://blog.csdn.net/liang19890820/article/details/51994441
		例如：
				RESOURCES += Resource/resource.qrc
				
	RC_FILE
		指定应用程序资源文件的名称。这个变量的值通常是由qmake和qmake.conf处理，很少需要进行修改。
		例如：
			RC_FILE += myapp.cpp
			
	RC_IOCONS
		仅适用于Windows，指定的图标应该包含在一个生成的.rc文件里。如果RC_FILE和RES_FILE变量都没有设置这才可利用。
		例如：
			RC_IOCONS = myapp.ico
	...
	TEMPLATE
		...
		aux			创建一个不建任何东西的Makefile。如果没有编译器需要被调用来创建目标，比如你的项目使用解释型
					语言写的，使用此功能。注：此模板类型只能用于Makefile-based生成器。特别是，它不会工作在vcxproj和Xcode生成器。
